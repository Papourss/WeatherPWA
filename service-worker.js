...
self.addEventListener("install", (event) => {
    event.waitUntil(
        (async () => {
            ...
            // the response isn't fulfilled from the HTTP cache; it will be from the network.
        ...
        event.respondWith(
            (async () => {
                try {
                    ...
                    // Catch is only triggered if an exception is thrown, likely due to a network error.
                    // If fetch() returns a valid HTTP response with a response code in the 4xx or 5xx range,
